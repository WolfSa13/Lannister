...
def requests_function(event):
    ...
    if action_id == 'request_start_menu':
        ...
        if created_request is not None:
            ...
            RequestHistoryQuery.add_history(data, request_id=created_request['id'], editor=creator['full_name'])
            ...
            view = request_created_successfully_modal(created_request['id'])
            data_creator_modal = {
        ...
        old_request = RequestQuery.get_requests(request_id=request_id)[0]
        updated_request = RequestQuery.update_request(request_id, data)
